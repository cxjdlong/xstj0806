...
- **段2 电话号码**：同编码规则。
- **段3 姓名**：只有 1 个，**可以不填**。
- **重复自动载入（关键）**：输入的**编码或电话已存在**时，自动把该联系人的现有资料载入输入框（进入修改模式），**重复项字体/边框标红**并顶部红条提示，**提交时二次确认覆盖**。

### 三、备份
- **完全导出**：导出全部联系人 Excel 到手机（`本机/Download/通讯录备份/`），**弹窗显示真实路径**。
- **全部备份**：导出 Excel + 写一条本地快照（备份列表里可「恢复」）。
