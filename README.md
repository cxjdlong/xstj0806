...
- **段2 电话号码**：同编码规则。
- **段3 姓名**：只有 1 个，**可以不填**。
- **重复处理（关键）**：输入的**编码或电话已存在**时 ——
  - 打字过程中**只在顶部显示黄色软提示 + 输入框标红，绝不打断/覆盖**输入；
  - 点提示里的「载入现有资料修改」可立即载入；
  - **离开输入框**（或直接提交）时会自动载入该联系人的现有资料，进入修改模式，**提交时二次确认覆盖**（确认框里显示将被覆盖的原资料）；修改模式下可点「改为新增」脱离。
  - 过短的值不参与查重（编码 <3 位、电话 <5 位忽略），避免打字到一半误命中。
