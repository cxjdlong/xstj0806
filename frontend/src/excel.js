...
export function parseWorkbook(arrayBuffer) {
  ...
  for (const name of wb.SheetNames) {
    ...
    if (idx.name < 0 && !idx.codes.length && !idx.phones.length) continue // 表头不认识，跳过
    for (let r = 1; r < rows.length; r++) {
      ...
      if (!codes.length && !phones.length && !nm) continue
      out.push({ codes, phones, name: nm })
    }
  }
  return out
}
